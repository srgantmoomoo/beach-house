...
public abstract class DropDownList extends HorizontalContainer {
	...
	public DropDownList (IEnumSetting setting, ThemeTuple theme, boolean container, boolean allowSearch, ITextFieldKeys keys, IScrollSize popupSize, Consumer<IFixedComponent> popupAdder) {
		...
		com.lukflug.panelstudio.widget.TextField textField=new TextField(new IStringSetting() {
			@Override
		...
		com.lukflug.panelstudio.widget.Button<Void> title=new com.lukflug.panelstudio.widget.Button<Void>(new Labeled("",null,()->false),()->null,popupTheme.getButtonRenderer(Void.class,false));
		com.lukflug.panelstudio.widget.RadioButton content=new RadioButton(new IEnumSetting() {
		...
		com.lukflug.panelstudio.widget.Button<Void> button=new com.lukflug.panelstudio.widget.Button<Void>(new Labeled(null,null,()->true),()->null,theme.getSmallButtonRenderer(ITheme.DOWN,container)) {
			@Override
			public void handleButton (Context context, int button) {
				super.handleButton(context,button);
